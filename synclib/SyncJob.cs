...
namespace Johnshope.SyncLib {
	...
	public class SyncJob {
		...
		public void Directory(IDirectory sdir, IDirectory ddir) {
			...
			var slist = sdir.List().Where(file => !Johnshope.SyncLib.Paths.Match(ExcludePatterns, file.RelativePath)).ToList();
			var dlist = ddir.List();
				...
				var t = new Task(() => {
					...
					if (src.Class == ObjectClass.File) {
						/*if (Verbose && dest != null) {
							Johnshope.SyncLib.Log.CyanText(src.Name + ":    " + src.Changed.ToShortDateString() + "-" + src.Changed.ToShortTimeString() + " => " +
		...
		public void Directory(Uri src, Uri dest) {
			try {
				...
				var sdir = Johnshope.SyncLib.Directory.Parse(src, this);
				var ddir = Johnshope.SyncLib.Directory.Parse(dest, this);
